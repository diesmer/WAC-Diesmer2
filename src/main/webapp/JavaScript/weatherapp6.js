 ...
 function showWeather(latitude, longitude, city){
	 if(sessionStorage.getItem(city) == null || Date.now() - new Date(JSON.parse(sessionStorage.getItem(city+"timer"))) >= 600000 ) {
		...
		myJson.calltimer = currentdate;
		sessionStorage.setItem(city+"timer", myJson.calltimer);
		var temp = myJson.main.temp - 271;
		 
		 sessionStorage.setItem(city, city);
         
	     document.querySelector("#temp").append(temp.toFixed(1));
		 sessionStorage.setItem(city + " temp", temp.toFixed(1));
		 
	     document.querySelector("#luchtvocht").append(myJson.main.humidity);
		 sessionStorage.setItem(city + " vocht", myJson.main.humidity);
		 
	     document.querySelector("#windsnel").append(myJson.wind.speed);
		 sessionStorage.setItem(city + " windsnel", myJson.wind.speed);
		 
	     document.querySelector("#windricht").append(windrichting(myJson.wind.deg));
		 sessionStorage.setItem(city + " windricht", myJson.wind.deg);
		 
	     document.querySelector("#opgang").append(gettime(myJson.sys.sunrise));
		 sessionStorage.setItem(city + " opgang", myJson.sys.sunrise);
		 
	     document.querySelector("#ondergang").append(gettime(myJson.sys.sunset));
		 sessionStorage.setItem(city + " ondergang", myJson.sys.sunrise);
	     
		console.log(city + " uit api!");
	 });
		} else {
			document.querySelector("#locatieStad").innerHTML = "Het weer in " + sessionStorage.getItem(city);
			
		    document.querySelector("#temp").innerHTML = "Temperatuur: " + sessionStorage.getItem(city + " temp");
		    
		    document.querySelector("#luchtvocht").innerHTML = "Luchtvochtigheid: " + sessionStorage.getItem(city + " vocht");
			 
		    document.querySelector("#windsnel").innerHTML = "Windsnelheid: " + sessionStorage.getItem(city + " windsnel");
			 
		    document.querySelector("#windricht").innerHTML = "Windrichting: " + sessionStorage.getItem(city + " windricht");
			 
		    document.querySelector("#opgang").innerHTML = "Zonsopgang: " + sessionStorage.getItem(city + " opgang");
			 
		    document.querySelector("#ondergang").innerHTML = "Zonsondergang: " + sessionStorage.getItem(city + " ondergang");
		    
			console.log(city + " uit sessionstorage!");
		}
